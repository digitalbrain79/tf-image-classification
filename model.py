...
def make_network(x, y):
    ...
    W_conv1 = weight_variable([3, 3, FLAGS.depth, FLAGS.num_map1])
    b_conv1 = bias_variable([FLAGS.num_map1])

    h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)
    h_pool1 = max_pool(h_conv1, 2, 2, 1)

    W_conv2 = weight_variable([3, 3, FLAGS.num_map1, FLAGS.num_map2])
    b_conv2 = bias_variable([FLAGS.num_map2])
    ...
    W_fc1 = weight_variable([height * width * FLAGS.num_map2, FLAGS.num_fc])
    b_fc1 = bias_variable([FLAGS.num_fc])

    h_pool2_flat = tf.reshape(h_pool2, [-1, height * width * FLAGS.num_map2])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

    W_fc2 = weight_variable([FLAGS.num_fc, FLAGS.num_class])
    b_fc2 = bias_variable([FLAGS.num_class])
